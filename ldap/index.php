...
$justthese = array( 'uid', 'displayname', 'cn', 'sn', 'givenname',
                    ...
                    'exeterathensaccount','exeterprintbillingcode' );

$printbillingcode = array ('exeterprintbillingcode');
...
$lookup = array( 'uid'                    	=>  'Username',
		 ...
		 'exeterprintbillingcode'	=>  'Print Services billing preference',
...
$update = array( 'uid'                    =>  '<a href="javascript:void(window.open(\'update-username.html\',\'_190x240\',\'height=600,width=500\'));">Further details</a>',
                 ...
                 'cn'                     =>  '<a href="javascript:void(window.open(\'update-name.html\',\'_190x240\',\'height=600,width=500\'));">Further details</a>',
                 ...
                 'edupersonnickname'      =>  '<a href="javascript:void(window.open(\'update-name.html\',\'_190x240\',\'height=600,width=500\'));">Further details</a>',
		 ...
		 'exeterprintbillingcode' =>  'EVALUATEPRINT',
...
// Connect to LDAP
global $content;
$content .="<script type='text/javascript'>function disable_input() {        if( document.print.exeterprintbillingcode.value == 'other' ) {                document.print.other.style.visibility='visible';  document.print.other.disabled = false;    }else { document.print.other.style.visibility='hidden';  }  }</script>";
...
if ($ds) { 
	...
	if ($_POST[$thisisit]=="billable"){
	   
	  $emptyprintbillingcode["exeterprintbillingcode"] = array();
	  $mr = ldap_modify($ds, $me, $emptyprintbillingcode);
	  
	  if ($mr) {
	    $content .= "<b>... Success!</b></p>";
            $content .= "<p><b>Please note Changes made here will be applied to the print system overnight. \n" .
            "You will be able to print using your new settings from tomorrow.</b></p>\n";
          } else {
	  ...
	  if (($thisisit=="exeterprintbillingcode" && (!(check($that[$thisisit]))))) {
	      $content .="<p>Sorry, an invalid billing code was found. Please try again!</p>"; 
              ...
              if ($length > $maxlength || $length < $minlength) {
                  ...
                  if ($mr) {
                      $content .= "<b>... Success!</b></p>";
                      if ($thisisit=="exeterprintbillingcode") {
                          $content .= "<p><b>Please note Changes made here will be applied to the print system overnight. \n" .
                                      "You will be able to print using your new settings from tomorrow.</b></p>\n";
                      }
                  } else {
    ...
    for ($i=0; $i<$info["count"]; $i++) {
        ...
        if (!(in_array($data, $printbillingcode))) {
                $detail = displayprint('exeterprintbillingcode','billable');
		$content .= '<tr><td><b><i>' . $lookup['exeterprintbillingcode'] . '</i></b></td><td>' . 'billable' . '</td><td align="center">' . $detail . '</td></tr>';
        }
